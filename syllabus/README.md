...
- **Env:** `DODO_PAYMENTS_API_KEY`, `DODO_ENV=test|live`,
  `DODO_PRODUCT_ENROLLED/HONOR_ROLL/EXTRA_CREDIT`, `DODO_WEBHOOK_SECRET`.
  Without keys/products, checkout fails closed with a clear message.
- Flow: plan → server creates Checkout Session → browser redirects to Dodo's
  hosted checkout → returns to `/billing/return` → server confirms session
  status via Dodo API → tier activates. `POST /api/billing/webhook` is backup.
